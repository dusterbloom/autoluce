...
from autoluce.coordination import Claim, Job
from autoluce.source_layout import SourceLayout
from autoluce.test_drive import _lease
...
class LocalExperimentExecutor:
    """Execute only the fixed autoluce experiment pipeline, never queued shell."""

    def __init__(self, *, simulate: bool = False, lock_path: str = "/tmp/autoluce-gpu.lock") -> None:
            ...
            with _lease(self.lock_path):
                for backend in claim.candidate.backends:
                    ...
                    env.update({
                        ...
                        "AUTOLUCE_BENCHMARKS": claim.candidate.model,
                        "AUTOLUCE_BUILD_JOBS": str(min(4, int(env.get("AUTOLUCE_BUILD_JOBS", "4")))),
                        "AUTOLUCE_BUILD_SUBDIR": f"build-{backend}",
                        "AUTOLUCE_EXPERIMENT_PATCH": patch_name,
                    })
                    process = subprocess.run(
                        [sys.executable, "-m", "autoluce.loop.agent_loop", "--dry-run"],
