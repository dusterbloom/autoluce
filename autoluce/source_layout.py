...
class SourceLayout:
    ...
    def resolve(cls, root: Path = ROOT, manifest_path: Path = DEFAULT_MANIFEST) -> "SourceLayout":
        ...
        override = os.environ.get("AUTOLUCE_SOURCE_ROOT")
    ...
    def require_capability(self, capability: str) -> None:
        if capability not in self.manifest.capabilities:
            raise RuntimeError(
                ...
                "Run `uv run autoluce source status` and use a product-runtime adapter."
            )

    def build_dir(self, backend: str) -> Path:
        subdir = os.environ.get("AUTOLUCE_BUILD_SUBDIR", f"build-{backend}")
