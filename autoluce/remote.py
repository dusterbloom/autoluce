...
class SSHWorker:
    ...
    def run_harness(self, args: list[str], env: dict[str, str] | None = None, timeout: int = 7200) -> dict[str, Any]:
        ...
        command = [f"{root}/.tools/uv", "run", "python", "-m", "autoluce.bench.harness", *args, "--json"]
        env_args = ["env", "AUTOLUCE_REMOTE_WORKER=1", f"AUTOLUCE_BUILD_JOBS={self.target.build_jobs}"]
