"""
Shadow bench for autoluce v2: the benchmark workload is the user's OWN local
traffic, and the quality oracle is KL divergence on their own prompts.

Flow:
  1. `autoluce shadow proxy`  -- reverse proxy in front of llama-server; tees the
     prompt of every /completion | /chat/completions POST to a local JSONL log.
  2. `autoluce shadow build`  -- turns the recent log into benchmarks/shadow.json
     (smoke.json schema) + a kl_text file under the shadow dir.
  3. `autoluce kl-base shadow` then `AUTOLUCE_BENCHMARKS=shadow autoluce run`.

Privacy: everything stays on-disk local. Prompts live under ~/.autoluce/shadow
...
DEFAULT_SHADOW_DIR = Path.home() / ".autoluce" / "shadow"
TEMPLATE_BENCHMARK = "smoke"


def shadow_dir() -> Path:
    """Prompt log + kl_text live here; overridable via AUTOLUCE_SHADOW_DIR."""
    return Path(os.environ.get("AUTOLUCE_SHADOW_DIR", DEFAULT_SHADOW_DIR))
...
def cmd_build() -> None:
    ...
    if not prompts:
        print("shadow build: no prompts in the last window; run `autoluce shadow proxy` "
    ...
    print("next:")
    print("  uv run autoluce kl-base shadow")
    print("  AUTOLUCE_BENCHMARKS=shadow uv run autoluce run")
