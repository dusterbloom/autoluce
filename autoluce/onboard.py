"""Install a user-local autoluce launcher on an SSH target."""
...
launcher.write_text(
    "#!/bin/sh\n"
    f"export AUTOLUCE_DEFAULT_TARGET={target_name}\n"
    f'exec "{root}/.tools/uv" run --no-dev --project "{root}" autoluce "$@"\n'
)
...
def onboard(target: TargetConfig, local_name: str = "lucebox3") -> dict:
    ...
    return {
        ...
        "next": [
            f"ssh {target.host}",
            "autoluce test-drive",
            "autoluce test-drive --live",
        ],
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Install a simple user-local autoluce experience on an SSH target")
    ...
    if args.json:
        ...
        return
    print("autoluce onboarding complete")
