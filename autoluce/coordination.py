"""Small, deterministic coordination core for a distributed autoluce team.
...
class FleetService:
    ...
    def submit(self, request: CandidateRequest) -> Job:
        ...
        def apply(state: dict) -> Job:
            ...
            if not eligible:
                label = "+".join(backends)
                raise NoCompatibleWorkerError(f"No idle machine supports {label}. Try `autoluce status` or resume a machine.")
