...
DEFAULT_TEAM_CONFIG = Path("~/.config/autoluce/team.json").expanduser()
...
class TeamConnection:
    ...
    @classmethod
    def path(cls) -> Path:
        return Path(os.environ.get("AUTOLUCE_TEAM_CONFIG", DEFAULT_TEAM_CONFIG)).expanduser()

    @classmethod
    def load(cls) -> "TeamConnection | None":
        url = os.environ.get("AUTOLUCE_COORDINATOR_URL")
        token = os.environ.get("AUTOLUCE_COORDINATOR_TOKEN")
        if url:
            if not token:
                raise ValueError("AUTOLUCE_COORDINATOR_TOKEN is required for a shared coordinator")
