"""
KL-to-baseline quality oracle (Tier-1) for autoluce v2.
...
`autoluce kl-base` fails closed until product quality capture is implemented
...
def check_kl(benchmark_name: str, model, extra: list[str] | None = None, timeout: int = 3600) -> dict[str, float]:
    ...
    if not base.exists():
        raise RuntimeError(
            ...
            f"generate it from the baseline build with `uv run autoluce kl-base {benchmark_name}`"
        )
