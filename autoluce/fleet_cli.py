...
from autoluce.coordination import CandidateRequest, FileCoordinationRepository, FleetService, JoinRequest
from autoluce.coordinator_http import CoordinatorClient
from autoluce.team_config import TeamConnection
from autoluce.team_worker import LocalExperimentExecutor, TeamWorker
...
def _machine_id() -> str:
    override = os.environ.get("AUTOLUCE_MACHINE_ID")
    if override:
        return override
...
def _worker_id(service: FleetService | CoordinatorClient, explicit: str | None) -> str:
    ...
    if not matches:
        raise ValueError("this machine has not joined; run `autoluce join`")
    return matches[0]


def _parser(action: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog=f"autoluce {action}")
    if action == "join":
        ...
        parser.add_argument("--lock-path", default="/tmp/autoluce-gpu.lock")
    else:
        parser.add_argument("--worker")
    return parser
...
def main(action: str | None = None, argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        raise SystemExit(f"autoluce: {error}") from error
