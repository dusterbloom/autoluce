...
DEFAULT_TARGETS_PATH = Path("~/.config/autoluce/targets.toml").expanduser()
...
class TargetConfig:
    ...
    lock_path: str = "/tmp/autoluce-gpu.lock"
    build_jobs: int = 4

    @classmethod
    def load(cls, name: str, path: Path | None = None) -> "TargetConfig":
        path = Path(os.environ.get("AUTOLUCE_TARGETS_FILE", path or DEFAULT_TARGETS_PATH)).expanduser()
        raw: dict = {}
        if path.exists():
            raw = tomllib.loads(path.read_text()).get("targets", {}).get(name, {})

        prefix = "AUTOLUCE_TARGET_"
        ...
        lock_path = os.environ.get(prefix + "LOCK_PATH", raw.get("lock_path", "/tmp/autoluce-gpu.lock"))
