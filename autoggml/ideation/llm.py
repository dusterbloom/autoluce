...
(OPENAI_BASE_URL + OPENAI_API_KEY + AUTOGGML_MODEL), so 'local vs cloud' is a config
...
DEFAULT_MODEL = "gpt-4o-mini"   # cloud-friendly default; override via AUTOGGML_MODEL for local
...
def config_from_env(env: dict[str, str] | None = None) -> LLMConfig | None:
    ...
    cloud-friendly default; set OPENAI_API_KEY / AUTOGGML_MODEL to override.
    """
    ...
    return LLMConfig(
        ...
        model=env.get("AUTOGGML_MODEL", DEFAULT_MODEL),
    )
    ...
    **params,
    ...
    if config is None:
        raise RuntimeError(
            ...
            "(OPENAI_API_KEY for cloud, AUTOGGML_MODEL to override the model)."
        )
