...
from autoggml.agent_config import AgentIdentity
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="autoggml agent", description="Cooperate and compete on bounded research tasks")
...
def _agent_id(explicit: str | None) -> str:
    ...
    if identity is None:
        raise ValueError("no default agent identity; run `autoggml agent join` or pass --agent")
    ...
    try:
        if args.command == "join":
            ...
            value = asdict(gateway.create_challenge(ChallengeRequest(
                ...
                base_commit=_commit(), implementation_slots=args.slots,
            ...
            if not args.no_worktree:
                path = ensure_worktree(ROOT, f"agent-{task.task_id}", context.challenge.base_commit)
        ...
        raise SystemExit(f"autoggml agent: {error}") from error
